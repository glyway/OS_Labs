...
bool execute_internal_command(char *tokens[], int token_count, char *output_file, bool append_output)
{
    ...
    {                                                                                                                                   // Вывод содержания каталога
        ...
        DIR *dir = ((token_count == 1 || (token_count > 1 && !!output_file && tokens[1] == NULL)) ? opendir(".") : opendir(tokens[1])); // Изменяет папку при данных аргументах
...
{
    // Мщем операторы переопределения ввода/вывода
...
{                                          // Обрабатываем запрос от пользователя
    ...
    update_input_output(tokens, token_count); // Проверяем завпрос на наличие input/output file

    if (token_count > 0)
    {
        int output_fd = -1;
        int oldoutput_fd = dup(STDOUT_FILENO); // Дуюликация консольного вывода
        if (output_file)
        {
